...
// we will change inner relation name to metafields
trait MetableTrait // extends Plank\Metable\Metable;
{
    /**
    ...
    public function setMeta($key, $value)
    {
        ...
        if ($this->relationLoaded('metafields')) {
            $this->metafields[$key] = $meta;
        }
    }
    ...
    {
        $this->metafields()->delete();
        $this->setRelation('metafields', $this->makeMeta()->newCollection([]));
    }
    ...
    {
        $q->whereHas('metafields', function (Builder $q) use ($key) {
    ...
    {
        $q->whereHas('metafields', function (Builder $q) use ($keys) {
            $q->whereIn('key', $keys);
        }, '=', count($keys));
    }
    ...
    public function scopeWhereMeta(Builder $q, $key, $operator, $value = null)
    {
        ...
        $q->whereHas('metafields', function (Builder $q) use ($key, $operator, $value) {
            $q->where('key', $key);
    ...
    public function scopeWhereMetaIn(Builder $q, $key, array $values)
    {
        ...
        $q->whereHas('metafields', function (Builder $q) use ($key, $values) {
            $q->where('key', $key);
    ...
    {
        if (!$this->relationLoaded('metafields')) {
            $this->setRelation('metafields', $this->metafields()->get());
        }

        return $this->getRelation('metafields');
    }
